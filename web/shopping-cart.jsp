...
<%
    ...
    ProductVariation variation = productDAO.getProductVariation(0);
    
    double total = 0;
    for(Cart_item c : cal){
        total += productDAO.getProductVariation(c.getProduct_id()).getPrice() * c.getQuantity();
    }
%>
<!DOCTYPE html>
<html lang="en">
                ...
                <div class="row">
                    <div class="col-lg-8">
                        <div class="cart_items">
                            ...
                            <% if(msg != null){ %>
                            <h6 style="color: red; padding-bottom: 5px;" ><%= msg %></h6>
                            <% } %>
                                    ...
                                    <tbody>
                                        ...
                                        <tr style="border: 1px solid #cccccc;background-color: white; padding: 10px;">
                                            <th scope="row">
                                                <form action="deletecartitem" method="get">
                                                    <input type="hidden" name="id" value="<%= c.getCart_item_id() %>">
                                                    <input type="submit" value="Remove"
                                                           style="border: 1px solid #cccccc;background-color: white; padding: 10px;">
                                                </form>
                                            </th>
                                            <td>
                                                <div class="media">
                                                    <div class="d-flex" >
                                                        <img style="width: 100px" src="img/product/<%= productDAO.getProductVariation(c.getProduct_id()).getProduct().getCoverImg()  %>" alt="">
                                                    </div>
                                                    <div class="media-body">
                                                        <h4 class="pb-1" ><%= productDAO.getProductVariation(c.getProduct_id()).getProduct().getProductName() %></h4>
                                                        
                                                        <h4><%= productDAO.getProductVariation(c.getProduct_id()).getColor() + "-" +  productDAO.getProductVariation(c.getProduct_id()).getSize()%></h4>
                                                    </div>
                                                </div>
                                            </td>
                                            <td><p class="red"><%= productDAO.getProductVariation(c.getProduct_id()).getPrice() %></p></td>
                                            <td>
                                                <!--Precision checking for price-->
                                                <script>
                                                    document.addEventListener('DOMContentLoaded', () => {
                                                        ...
                                                        PlusButton.addEventListener('click', () => {
                                                            if(resultP.value < <%= productDAO.getProductVariation(c.getProduct_id()).getQuantity() %> ){
                                                                resultP.value++;
                                                                UpdateinputElement.value++;
                                                                updateButton();
                                                            }
                                                        ...
                                                        var calculatedPrice = document.getElementById('<%= "total" + c.getCart_item_id() %>');
                                                        var value = parseDouble(calculatedPrice.value);
                                                        calculatedPrice.textContent = value.toPrecision(3);
                                                    });
                                                    
                                                    document.addEventListener('DOMContentLoaded', () => {
                                                        var calculatedPrice = document.getElementById('<%= "total" + c.getCart_item_id() %>');
                                                        var value = parseFloat(calculatedPrice.textContent);
                                                        if (!isNaN(value)) {
                                                            calculatedPrice.textContent = value.toPrecision(3);
                                                        } else {
                                                    ...
                                                    function updateButton() {
                                                        var textInput = document.getElementById('<%= c.getCart_item_id() %>');
                                                        var btn = document.getElementById('update-p');
                                                ...
                                                <div class="quantity">
                                                    <h6>Quantity</h6>
                                                    <div class="custom">
                                                        <button id="<%= "btnMinus" + c.getCart_item_id() %>"
                                                            class="reduced items-count" type="button"><i class="icon_minus-06"></i></button>
                                                            <input onchange="updateButton()" class="quantity-input" type="text" name="qty" id="<%= c.getCart_item_id() %>" maxlength="2" value="<%= c.getQuantity() %>" title="Quantity:" class="input-text qty">
                                                        <button id="<%= "btnPlus" + c.getCart_item_id() %>"
                                                            class="increase items-count" type="button"><i class="icon_plus"></i></button>
                                                    </div>
                                                </div>
                                            </td>
                                            <td><p id="<%= "total" + c.getCart_item_id() %>"><%= c.getQuantity() * productDAO.getProductVariation(c.getProduct_id()).getPrice() %></p></td>
                                        ...
                                        <div class="btn-update text-center mt-5">
                                            <form action="updatecartitem" method="post">
                                                <!--Hidden input fields for update-->
                                                <% for(Cart_item c : cal){  %>
                                                <input name=<%= "quantity" + c.getCart_item_id() %> id="<%= "update-input" + c.getCart_item_id() %>" type="hidden" value="<%= c.getQuantity() %>">
                                                <% } %>
                                                <button type="submit" id="update-cart-btn" style="background: none;
                                                                border: none;
                                                                padding: 0;
                                                                margin: 0;
                                                                color: transparent;
                                                                cursor: pointer;"
                                                                >   
                                                    <p id="update-p" style="background-color: black; border: 1px solid #cccccc;padding:15px;margin:20px;color: white;font-size: 18px;font-weight: 600;">Update Cart</p>
                                                </button>
                                            </form>
                                        </div>
                            </div>
                                                ...
                                                <!--Precision checking for total price-->
                                                <script>
                                                    document.addEventListener('DOMContentLoaded', () => {
                                                        var calculatedPrice = document.getElementById('total-price');
                                                        var value = parseFloat(calculatedPrice.textContent);
                                                        if (!isNaN(value)) {
                                                            calculatedPrice.textContent = value.toPrecision(3);
                                                        } else {
                    ...
                    <div class="col-lg-4" style="margin-top: 6%;">
                        <div class="cart_totals_area">
                            ...
                            <div class="total_amount row m0 row_disable">
                                ...
                                <div style="font-size: 24px;" class="float-right" id="total-price">
                                    <%= total %>
                                </div>
                            </div>
                        </div>
                        <form action="checkout" method="post">
                            <button style="background-color: red;" type="submit" value="submit" class="btn subs_btn form-control">Proceed to checkout</button>   
                        </form>
                    </div>
                </div>
            </div>
